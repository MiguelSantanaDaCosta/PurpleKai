#!/usr/bin/env python3
"""
Script de demonstração profissional do PurpleKai - Modo Ao Vivo
Autor: Miguel Santana
Descrição: Abre o nano, digita o código linha por linha (simula codificação ao vivo)
...
║    ╚═════╝ ╚═╝  ╚═╝ ╚═════╝ ╚═╝     ╚══════╝╚══════╝       ║
║                                                              ║
║           PurpleKai - Demonstração Ao Vivo                  ║
...
def digitar_no_nano(conteudo_linhas, intervalo_base=0.2, variacao=0.1):
    """
    Envia texto linha por linha para o nano.
    - intervalo_base: pausa média entre linhas (segundos)
    - variacao: variação aleatória para simular hesitação humana
    As linhas são enviadas exatamente como estão (com espaços de indentação).
    """
    time.sleep(1.5)  # Aguarda o nano abrir
    for idx, linha in enumerate(conteudo_linhas):
        # Pequena pausa variável antes de cada linha (exceto a primeira)
        if idx > 0:
        ...
        if linha.strip() == "":
            ...
            # Envia a linha com indentação (espaços já estão na string)
            subprocess.run(["wtype", linha])
            time.sleep(0.05)  # pequena pausa entre o texto e o Enter
            subprocess.run(["wtype", "-kp", "enter"])
    
    # Salvar (Ctrl+O), confirmar, sair (Ctrl+X)
    time.sleep(0.5)
    subprocess.run(["wtype", "-k", "ctrl+o"])
    time.sleep(0.3)
    subprocess.run(["wtype", "-kp", "enter"])   # confirma nome do arquivo
    time.sleep(0.3)
    subprocess.run(["wtype", "-k", "ctrl+x"])
    time.sleep(0.5)

def executar_ao_vivo(velocidade="normal"):
    """Configura a velocidade de digitação"""
    velocidades = {
        "lento": (0.4, 0.15),
        "normal": (0.2, 0.1),
        "rapido": (0.1, 0.05)
    ...
    # Conteúdo linha a linha com indentação adequada
    linhas = [
        'laisser a = 10;',
        'laisser b = 20;',
        'laisser soma = a + b;',
    ...
    comando = f'bash -c "nano {script_path}; echo; ./purplekai {script_path}; read -p \'Pressione Enter para fechar...\'"'
    ...
    time.sleep(2)  # Aguarda o nano abrir
    digitar_no_nano(linhas, intervalo_base, variacao)
...
def main():
    parser = argparse.ArgumentParser(description="PurpleKai Demonstração Ao Vivo (digitação em nano)")
    parser.add_argument("--live", action="store_true", help="Modo ao vivo: abre nano e digita o código")
    ...
    print(BANNER)
    if args.live:
        print_step(f"Iniciando demonstração ao vivo (velocidade: {args.velocidade})...", "info")
